...
function generateHook(apis, types, config) {
    ...
    `
        ...
        const returnType = () => {
            if (hasSwitchablePagination)
                return `:  QueryResult<T, Infinite> & ReturnType<typeof useInfiniteQuery<T>>`;
            return "";
        };
        const params = [
            `${isGet ? TVariables : ""}`,
        ...
        result += `export const ${hookName} = ${hasSwitchablePagination ? `<T = ${TData}, Infinite extends boolean = false>` : ""}${!isGet ? `<TExtra>` : ""}(
      ${params.join("\n      ")}
    )${returnType()} => {`;
        if (isGet) {
            result += `
      const { key, fun } = ${hookName}.info(${getParamsString({
            ...
            const infiniteBlock = (hasEnable, typeAssign) => {
                var _a, _b;
                return `
        const {
          data: { pages } = {},
